        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 h-full flex flex-col pt-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 flex-1 min-h-0">
                <div className="lg:col-span-1 flex flex-col">
                    <div className="bg-transparent flex flex-col h-full">
                        ...
                        <div
                            ...
                            className={`flex-1 border-[0.5px] border-dashed border-border flex flex-col items-center justify-center p-6 text-center transition-colors cursor-pointer group mb-8 ${file ? 'bg-foreground/5' : 'hover:bg-foreground/5'}`}
                        >
                        ...
                        {status && (
                            <div className="text-[10px] uppercase tracking-[0.1em] text-text-secondary mb-4 p-3 border border-border text-center">
                                {status}
                            </div>
                        )}
                        ...
                        <button
                            ...
                            className={`w-full border border-border text-[11px] tracking-[0.15em] text-foreground py-4 uppercase transition-colors duration-300 ${(!uploading && file) ? 'hover:bg-foreground hover:text-background' : 'opacity-50 cursor-not-allowed'}`}
                        >
                ...
                <div className="lg:col-span-2">
                    <div className="bg-transparent border border-border h-full flex flex-col transition-colors duration-300">
                        <div className="p-6 border-b border-border flex justify-between items-center transition-colors duration-300">
                            ...
                            <span className="text-[9px] tracking-[0.1em] text-text-secondary uppercase border border-border px-3 py-1 transition-colors duration-300">{candidates.length} Uploaded</span>
                        </div>
                        {candidates.length === 0 ? (
                            ...
                            <div className="flex-1 overflow-auto p-4">
                                {candidates.map((c, i) => (
                                    <div key={c.id} className="flex items-center justify-between py-4 border-b border-border last:border-b-0">
                                        ...
                                        <span className={`text-[10px] uppercase tracking-[0.1em] px-3 py-1 border ${c.status === 'completed' ? 'text-foreground border-foreground' : c.status === 'failed' ? 'text-red-500 border-red-500' : 'text-text-secondary border-border'
                                            }`}>
                                            {c.status}
                                        </span>
                                    </div>
