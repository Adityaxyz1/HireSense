...
```mermaid
flowchart LR
  subgraph FE["Applicant Portal (React)"]
    SU["Signup<br/>LinkedIn + GitHub URL"]
    OB["Onboarding form<br/>(primary landing)"]
  end
  subgraph BE["Backend (FastAPI)"]
    ST["routes/student.py<br/>update_applicant_profile"]
    GH["github_service.py<br/>FREE - Phase A"]
    LI["linkedin_service.py<br/>PAID/TBD - Phase B"]
  end
  DB[("applicant_profiles<br/>github_data / linkedin_data<br/>profile_completed")]
  GHAPI["api.github.com<br/>(public REST)"]
  LIAPI["LinkedIn provider<br/>MagicalAPI / Proxycurl"]
  subgraph REC["Recruiter Portal"]
    EV["Applicants / Candidates<br/>evaluation panel"]
  end
...
### 2. Mandatory onboarding gate (login → portal)
```mermaid
flowchart TD
  L["Applicant logs in"] --> R{"role == applicant?"}
  R -->|no| X["Recruiter / Admin area"]
  R -->|yes| C{"profile_completed?"}
  C -->|no| OB["Redirect to /student/onboarding<br/>HARD GATE"]
  OB --> F["Fill required fields + Save"]
  F --> C
  C -->|yes| P["Browse Jobs / My Applications / ATS Check"]
```

### 3. Profile enrichment sequence
```mermaid
sequenceDiagram
  participant A as Applicant
  participant FE as Onboarding (FE)
  participant BE as student.py
  participant GH as github_service
  participant DB as applicant_profiles
  participant R as Recruiter UI

  A->>FE: Fill profile (LinkedIn + GitHub URLs)
  FE->>BE: PUT /student/profile
  BE->>DB: save fields + profile_completed = true
  BE-->>GH: background fetch_github_profile(url)
  GH->>GH: GET /users/{u} + /repos (public API)
  GH->>DB: store github_data + profile_synced_at
  Note over BE,GH: LinkedIn fetch is identical in Phase B
  DB-->>R: realtime update -> evaluation panel shows GitHub/LinkedIn
```
